...
class INET_API IPv6Route : public cObject
{
  ...
  public:
    ...
    IPv6Route(IPv6Address destPrefix, int length, RouteSrc src, int rank) {
        _rt = NULL;
        _destPrefix = destPrefix;
        _length = length;
        _src = src;
    ...
    void setMetric(double metric)  { if (metric != _metric) { _metric = metric; changed(F_METRIC);} }
    void setAdminDist(unsigned int adminDist)  { if (adminDist != _adminDist) { _adminDist = adminDist; changed(F_ADMINDIST);} }
    void setRank(int rank) { if (rank != _rank) { _rank = rank; changed(F_RANK);} }
    ...
    double getMetric() const  {return _metric;}
...
class INET_API RoutingTable6 : public cSimpleModule, protected INotifiable, public ILifecycle
{
  ...
  protected:
    ...
    virtual IPv6Route *RPLcreateNewRoute(IPv6Address destPrefix, int prefixLength, IPv6Route::RouteSrc src, int rank);
    ...
    virtual void RPLaddRoute(IPv6Route *route);
    // helper for addRoute()
    static bool routeLessThan(const IPv6Route *a, const IPv6Route *b);
    static bool RPLrouteLessThan(const IPv6Route *a, const IPv6Route *b);
    // internal
  ...
  public:
    virtual IPv6Route *getRouteList()
    {
        return routeList[0];
    }
    /** @name Interfaces */
    ...
    const IPv6Address& RPLlookupDestCache(const IPv6Address& dest, int& outInterfaceId);
    /**
    ...
    const IPv6Route *firstRouteInList(const IPv6Address& dest);
    /**
     ...
     *  address and the router expiry time plus the simTime().
     */
    virtual void RPLaddDefaultRoute(const IPv6Address& dioSrcAddr, unsigned int ifID, double metric, double rank);
