...
IPv6Route *RoutingTable6::RPLcreateNewRoute(IPv6Address destPrefix, int prefixLength, IPv6Route::RouteSrc src, int metric)
{
    return new IPv6Route(destPrefix, prefixLength, src, metric);
}

const IPv6Address& RoutingTable6::RPLlookupDestCache(const IPv6Address& dest, int& outInterfaceId)
{
    Enter_Method("RPLlookupDestCache(%s)", dest.str().c_str());
...
void RoutingTable6::configureInterfaceForIPv6(InterfaceEntry *ie)
{
    ...
    //as a host?
    //COMMENTED BY SEER 19TH APR//ipv6IfData->setAdvSendAdvertisements(isrouter); //Added by WEI
    ...
    if (!ie->isLoopback())
        //COMMENTED BY SEER 19TH APR//addStaticRoute(IPv6Address::LINKLOCAL_PREFIX, 10, ie->getInterfaceId(), IPv6Address::UNSPECIFIED_ADDRESS);
        routeList.clear();
    if (ie->isMulticast())
    {
...
//The function name is not that proper and may be confusing. The main focus of it is just return the
//first element in the routelist.
const IPv6Route *RoutingTable6::firstRouteInList(const IPv6Address& dest)
{
    Enter_Method("firstRouteInList(%s)", dest.str().c_str());
    if(routeList.empty())
    {
        EV<<"No information about my next hop, do nothing."<<endl;
        return NULL;
    }
    // we'll just stop at the first match, because the table is sorted
    // by prefix lengths and metric (see addRoute())
    ...
    //******** for random parent setting
        //srand((unsigned)time(NULL));
       // int i = rand()%3;
    //int i = 0;
    //EV<<"the next hop we get from firstRouteInList:"<<routeList[i]->getNextHop()<<endl;
        return routeList[0];
...
void RoutingTable6::RPLaddDefaultRoute(const IPv6Address& nextHop, unsigned int ifID, double metric, double rank)
{
    // create route object
    IPv6Route *route = RPLcreateNewRoute(IPv6Address(), 0, IPv6Route::FROM_RPL, metric);
    route->setInterfaceId(ifID);
    route->setNextHop(nextHop);
    route->setMetric(metric); //FIXME:should be filled from interface metric
    route->setRank(rank);
    route->setAdminDist(IPv6Route::dStatic);
...
bool RoutingTable6::RPLrouteLessThan(const IPv6Route *a, const IPv6Route *b)
{
...
void RoutingTable6::RPLaddRoute(IPv6Route *route)
{
    route->setRoutingTable(this);
    std::vector<IPv6Route*>::iterator it;
    EV<<"From this dio:route.nextHop："<<route->getNextHop()<<endl;
    EV<<"From this dio, metric we calculated:route.metric："<<route->getMetric()<<endl;
//    if(!routeList.empty())
//    {
//        if(route->getRank() < routeList[0]->getRank())
//        {
//            routeList.clear();
//        }
//    }
    ...
    {
        if(route->getNextHop() == (*it)->getNextHop())
        {
            (*it)->setMetric(route->getMetric());
            break;
        }
    }
    EV<<"Currently, routelist length:"<<routeList.size()<<endl;
    if (it == routeList.end()) {
        routeList.push_back(route);
        EV<<"New added route:route->nexthop():"<<route->getNextHop()<<endl;
    }
    EV<<"Add finished, routelist length turn into:"<<routeList.size()<<endl;
    EV<<"routeList[0]->getNextHop(): before sorting:"<<routeList[0]->getNextHop()<<endl;
    EV<<"routeList[0]->getMetric()"<<routeList[0]->getMetric()<<endl;
    EV<<"routeList[0]->getRank()"<<routeList[0]->getRank()<<endl;

    // we keep entries sorted by rank of next hop in routeList, so that we can
    // stop at the first match when doing the rank comparison

    std::stable_sort(routeList.begin(), routeList.end(), RPLrouteLessThan);
    EV<<"After sorting, first Route:"<<routeList[0]->getNextHop()<<endl;
