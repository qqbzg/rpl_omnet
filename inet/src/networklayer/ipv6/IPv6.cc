...
#include "/Users/apple/Documents/omnetpp-4.6/samples/MiXiM/src/base/utils/MacToNetwControlInfo.h"
...
void IPv6::endService(cPacket *msg)
{
    ...
    if (msg->getArrivalGate()->isName("transportIn")
            ...
            || (msg->getArrivalGate()->isName("ndIn") && dynamic_cast<IPv6RPLMessage*>(msg))
    ...
    else
    {
        ...
        recordScalar("-ipv6-cInfo->getresidualEnergy",cInfo->getresidualEnergy());
        recordScalar("ipv6-cInfo->getrssi",cInfo->getRSSI());

        IPv6Datagram *datagram = check_and_cast<IPv6Datagram *>(msg);
        EV<<"datagram->getControlInfo:"<<datagram->getControlInfo()<<endl;
        datagram->setResidualenergy(cInfo->getresidualEnergy());
        recordScalar("datagram->setResidualenergy", datagram->getResidualenergy() );

        datagram->setParam(cInfo->getRSSI());
        bool fromHL = false;
        ...
        {
            EV<<"A datagram from "<<datagram->getSrcAddress()<<endl;
...
{
    EV<<"HandleMessage from HL:msg->getClassName()： "<<msg->getClassName()<<endl;
    ...
    EV<<"datagram src address:"<<datagram->getSrcAddress()<<endl;
   // packetlog.record(datagram->getSrcAddress());
    delete controlInfo;
    ...
    if (!destAddress.isMulticast() && rt->isLocalAddress(destAddress))
    {
        ...
        if (!destIE)
            {destIE = ift->getFirstLoopbackInterface();EV<<destIE<<endl;}
        ASSERT(destIE);
    }
    ...
    if (destIE != NULL)
    {
        if(!strcmp(msg->getClassName(),"DAO"))//(msg->getClassName()== "DAO")
        {
            MACAddress destMACAddress = datagram->getDestMACAddress();
            EV<<"My dest mac address is : "<<destMACAddress<<endl;
            EV<<"Source Addr:"<<datagram->getSrcAddress()<<endl;
            EV<<"unicast!!"<<endl;
            fragmentAndSend(datagram, destIE, destMACAddress, true);
        }
        else{
            EV<<"Source Addr:"<<datagram->getSrcAddress()<<endl;
            EV<<"broadcast!!"<<endl;
...
void IPv6::routePacket(IPv6Datagram *datagram, InterfaceEntry *destIE, bool fromHL)
{
    ...
    // Try to turn down the function of Tunneling.
    //*********************************************************************************
    ...
    if (interfaceId == -1)
        if ( !RPLdetermineOutputInterface(destAddress, nextHop, interfaceId, datagram, fromHL) )
    ...
    MACAddress macAddr = nd->RPLresolveNeighbour(nextHop, interfaceId); // might initiate NUD
...
void IPv6::routeMulticastPacket(IPv6Datagram *datagram, InterfaceEntry *destIE, InterfaceEntry *fromIE, bool fromHL)
{
    ...
    //MacToNetwControlInfo* cInfo = static_cast<MacToNetwControlInfo*>(datagram->getControlInfo());
    EV<<"datagram->getControlInfo():"<<datagram->getControlInfo()<<endl;
    EV<<"param I cared(RSSI/BER/SNR):"<<datagram->getParam()<<endl;
    EV<<"residualEnergy I cared:"<<datagram->getResidualenergy()<<endl;
...
void IPv6::localDeliver(IPv6Datagram *datagram)
{
    ...
    else if (protocol == IP_PROT_IPv6_ICMP && dynamic_cast<IPv6RPLMessage*>(packet))
        {
            EV << "RPL packet: passing it to ND module to handle\n";
            send(packet, "ndOut");
        }
#ifdef WITH_xMIPv6
...
cPacket *IPv6::decapsulate(IPv6Datagram *datagram)
{
    ...
    controlInfo->setParam(datagram->getParam());
    controlInfo->setResidualEnergy(datagram->getResidualenergy());
...
IPv6Datagram *IPv6::encapsulate(cPacket *transportPacket, IPv6ControlInfo *controlInfo)
{
    ...
    MACAddress destMAC = controlInfo->getDestMACAddr();
    datagram->setDestMACAddress(destMAC);
    ...
    datagram->setHopLimit(controlInfo->getHopLimit()>0 ? controlInfo->getHopLimit() : 2048); //FIXME use iface hop limit instead of 32?
...
void IPv6::fragmentAndSend(IPv6Datagram *datagram, InterfaceEntry *ie, const MACAddress& nextHopAddr, bool fromHL)
{
    ...
    EV<<"Source Address of the datagram:"<<datagram->getSrcAddress()<<endl;
            ...
            !datagram->getDestAddress().isSolicitedNodeMulticastAddress())
    {
        ...
        EV<<"after setting:"<<datagram->getSrcAddress()<<endl;
    #ifdef WITH_xMIPv6
    ...
    int mtu = ie->getMTU();
    EV<<"mtu:"<<mtu<<endl;
    EV<<"DATAGRAM BYTELENGTH:"<<datagram->getByteLength()<<endl;
...
bool IPv6::RPLdetermineOutputInterface(const IPv6Address& destAddress, IPv6Address& nextHop,
                                    int& interfaceId, IPv6Datagram* datagram, bool fromHL)
{
    // try destination cache
    //IPv6Address nextHop = rt->lookupDestCache(destAddress, interfaceId);
    nextHop = rt->RPLlookupDestCache(destAddress, interfaceId);

    if (interfaceId == -1)
        {
            // address not in destination cache: do longest prefix match in routing table
            EV << "Searching in routing table" << endl;
            const IPv6Route *route = rt->firstRouteInList(destAddress);
            if (!route)
            {
                if (rt->isRouter())
                {
                ...
                {
                    EV << "No added no match in routing table, passing datagram to Neighbour Discovery module for default router selection\n";
                    datagram->addPar("IPv6-fromHL") = fromHL;
                    send(datagram, "ndOut");
                }
                return false;
