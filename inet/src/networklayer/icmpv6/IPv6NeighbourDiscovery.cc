...
#include "IPv6RPLMessage_m.h"
#include "ObjectiveFun0.h" //*******"ObjectiveFun1.h"
...
#define MK_SEND_DIO 8
#define MK_SEND_DIS 9
#define MK_SEND_DAO 10
...
void IPv6NeighbourDiscovery::initialize(int stage)
{
    ...
    else if (stage == 3)
    {
        ...
        sinkAddress = getParentModule()->getParentModule()->getParentModule()->par("SinkNodeAddr");
        ...
        DISenabled = par ("DISenabled");
        MsgEnding = par("MsgEnding");
        ...
        DIO_Redun = 5;
        DIO_Supressed = 0;
        //NumDio = 0;   //received DIO numbers from certain parent/neighbor node no need to set as an independent
        //variable, it is just a relay variable which will be used one step further
        NumSelfDIO = 0;
        IsJoined = false;
        resetFlag = 0;
        myMACAddress = MACAddress();
        destMACAddress = MACAddress();
        rankchangeLog.setName("rankchangeLog");
        DIONumchangeLog.setName("DIONumchangeLog");
        parentLog.setName("parentLog");
        dio_c.setName("dio_c");
        ...
        if (rt6->isRouter())
            scheduleAt(simTime() + uniform(0, 0.005), msg); //Random Router bootup time (0,0.3)
        else
            scheduleAt(simTime() + uniform(0.995, 1), msg); //Random Host bootup time (0.4,1)
        ...
        NodeID = host->getId()-4;
        param_reference = 0.0;
        EV<<"The corresponded NodeID:"<<NodeID<<"\n"<<"Module ID:"<<getId()<<endl;
        if (NodeID == sinkAddress)
        {
            IsJoined = true;
            cModule* host = findHost();
            of0.Rank = 1;
            ...
            /* Here we need to print the root info
             * char buf2[100];
             * sprintf(buf2, "I am the root. ");
...
void IPv6NeighbourDiscovery::handleMessage(cMessage *msg)
{
    if (msg->isSelfMessage())
    {
        ...
        if (msg->getKind() == MK_SEND_DIO)
        {
            EV << "Sending DIO\n";
            sendDIO(msg);
        }
        else if (msg->getKind() == MK_SEND_DIS)
        {
            EV << "Preparing to send DIS\n";
            sendDIS(msg);
        }
        else if (msg->getKind() == MK_SEND_DAO)
        {
            EV << "Preparing to send DAO\n";
            sendDAO(msg);
        }
...
//        }
        else
        {
            ...
            EV<<"unrecognized timer"<<msg->getKind()<<endl;
        }
    }
    else if (dynamic_cast<ICMPv6Message *>(msg))
    {
        ...
        EV<<"controlInfo->getParam:"<<ctrlInfo->getParam()<<endl;
...
void IPv6NeighbourDiscovery::TrickleReset()
{
...
DIO *IPv6NeighbourDiscovery::createAndSendDIOPacket(const IPv6Address& destAddr, InterfaceEntry *ie)
{

    EV << "Create and send DIO invoked!\n";

    myIPv6Address = ie->ipv6Data()->getPreferredAddress(); //Interface Address
    myMACAddress = ie->getMacAddress();
    DIO *dio = new DIO("DIOpacket");
    dio->setInitialSrcAddr(myIPv6Address);
    dio->setInitialSrcMACAddr(myMACAddress);
    dio->setSrID(NodeID);
    EV<<"DIO Source Addr is: "<< dio->getInitialSrcAddr()<<endl;
    dio->setFinalDestAddr(destAddr) ;
    dio->setVersionNumber(1) ;
    dio->setRank(of0.Rank);
    EV<<"of0.rank"<<of0.Rank<<endl;
    //The next line is added 9th July, caring about the accumulative EXT
    //dio->setMetrics(of0.metrics);
    //*****The next two lines are only used by objective function fuzzy***
    //dio->setMetrics_t(off.metrics_t);
    //dio->setMetrics_plus(off.metrics_plus);  //****this is an alternative means by which may complicate the process

    //EV<<"of0.metrics:"<<of0.metrics<<endl;
    dio->setNumSent(fetchDIOIfEntry(ie)->numDIOSent);
    DIONumchangeLog.record(fetchDIOIfEntry(ie)->numDIOSent);
    dio->setDODAGID(sinkIPv6Address) ;
    EV<<"DODAGID is: "<<dio->getDODAGID()<<endl;
    dio->setGrounded(1) ;
    sendPacketToIPv6Module(dio, destAddr, myIPv6Address, ie->getInterfaceId());
    return dio;
}
simtime_t IPv6NeighbourDiscovery::scheduleNextDIOTransmissionInconsistency(cMessage *DIOTimer)
{
    DIOCurInt = DIONextInt;
    simtime_t interval = uniform(DIOCurInt/2, DIOCurInt);
    EV<<"Current Interval:"<<DIOCurInt<<endl;
    DIONextInt*=2;
    ...
    if(DIO_c < DIO_Redun)
    {
        scheduleAt(nextScheduledTime, DIOTimer);
        if(DIONextInt > double(DIOIMaxLength)) DIONextInt = DIOIMaxLength;
        return nextScheduledTime;
    }

//    simtime_t intervalInfinite = 300;
//    if(simTime() == MsgEnding)
//    {
//        nextScheduledTime = simTime() + intervalInfinite;
//    }
    else
    {
        recordScalar("diored", DIO_Redun);
//        cancelAndDelete(DIOTimer);
...
simtime_t IPv6NeighbourDiscovery::scheduleNextDIOTransmissionConsistency(cMessage *DIOTimer)
{
    cancelAndDelete(DIOTimer);
...
void IPv6NeighbourDiscovery::sendDIO(cMessage *DIOTimer1)
{
    InterfaceEntry *ie = (InterfaceEntry *)DIOTimer1->getContextPointer();
    EV<<"Message Kind for check:"<<DIOTimer1->getKind()<<endl;
    DIOIfEntry *dioIfEntry = fetchDIOIfEntry(ie);
    IPv6Address destAddr =  IPv6Address::ALL_ROUTERS_2;//IPv6Address("FF02::1A");
    dioIfEntry->numDIOSent++;
    NumSelfDIO = dioIfEntry->numDIOSent;
    createAndSendDIOPacket(destAddr, ie);
    ...
    EV << "Num DIO sent is: " << dioIfEntry->numDIOSent << endl;

    simtime_t nextScheduledTime = scheduleNextDIOTransmissionInconsistency(DIOTimer1);
    dioIfEntry->nextScheduledDIOTime = nextScheduledTime;
}

//Read before change of - note parameters in addparent function is different
void IPv6NeighbourDiscovery::processDIOPacket
(DIO *dio,IPv6ControlInfo *dioCtrlInfo)
{
    /**Added 9th Jan 2016 to get access to battery module.
     * Thus, the real-time battery info in the node can be collected and make into further use:
     * such as using as part of metrics...
     */
    //EV<<"Energy:"<<dioCtrlInfo->getResidualEnergy()<<endl;
    EV<<"DIO received from node "<<dio->getSrID()<<" with rank "<<dio->getRank()<<"!!!"<<endl;
    char buf1[100];
    int srID = dio->getSrID();

    /*USED BY OF 3 - ETX RSSI*/
    //EV<<"param_reference:"<<param_reference<<endl;

    //***OF 3 NEXT 2 LINE CALCULATE PARAMETERS
    //param_reference = (param_reference + log(dioCtrlInfo->getParam()))/2;//-10.0;//5.0;//(param_reference + log(dioCtrlInfo->getParam()))/2;
    //double rssi = (param_reference-log(dioCtrlInfo->getParam()))/120;//dioCtrlInfo->getParam();

    //EV<<"dio control param:"<<(reference-log(dioCtrlInfo->getParam()))/80<<endl;//dioCtrlInfo->getParam()<<" "<<log(dioCtrlInfo->getParam())<<endl;
    //reference-log(dioCtrlInfo->getParam())<<endl;
    /*USED BY OF 3 - ETX RSSI*/


    /*USED BY OF FUZZY - FUZZY CONTROL*/
    //**********************for objective function fuzzy*****************/
    //double metrics_plus = dioCtrlInfo->getResidualEnergy();
    //recordScalar("metrics_plus",metrics_plus);
    /*USED BY OF FUZZY - FUZZY CONTROL*/



    EV<<"dio mac addr:"<<dio->getInitialSrcMACAddr()<<endl;

    //***THE NEXT TWO LINES ARE ABOUT THE DATA USED IN THE *ACCUMULATIVE ETX* CALCULATION
    //***FUNC. ADDED 9TH JULY
    //int NumDio = dio->getNumSent();

    //***IT REPRESENTS THE METRIC OF THE CANDIDATE PARENT, USED BY *OBJECTIVE FUNC2AND3ANDFUZZ*
    //double metrics_t = dio->getMetrics();
    //EV<<"this dio contained metric:"<<metrics_t<<endl;

    //***THIS IS FOR *OBJECTIVE FUNCTION FUZZ*, *ADDICTIVE ETX*
    //double metrics_t_etx = dio->getMetrics_t();


    //***FOR *OBJECTIVE FUNCTION 0*, WE JUST RELATE RANK WITH THE HOP COUNT
    metrics = dio->getRank()+1;

    //***FOR *RECORDED ETX*, DEFINED IN THE BELOW BRANCHES RESPECTIVELY, UNCOMMENTED JUST EXPLANATION
    //metrics = of1.counterCal(srID,dio->getNumSent());


    //***THIS ONE IS USED FOR ACCUMULATIVE ETX, WHICH MAKES USE OF THE INFO FROM RCVD DIO MSG
    //***CODES BELOW REMAIN COMMENTED. JUST EXPLANATION IN GENERAL.
    //***THE CODE WORKS ARE IN THE INDIVIDUAL BRANCH BELOW.
    //double count = of0.counterCal(srID, metrics_t, NumDio);
    //metrics = of0.counterCal(srID, metrics_t, NumDio);
    //EV<<"count metrics:"<<metrics<<endl;

    sprintf(buf1,"A DIO message received from node[%d]\n",srID);
    cModule* host = findHost();
       if (NodeID == sinkAddress)
       {
           char buf2[100];
       ...
       {
           IPv6Address src = dio->getInitialSrcAddr();
           //first time a node join the DODAG
           if(!IsJoined)
           {
               //***BELOW IS FOR of0, UNCOMMENT IT WHEN NECESSARY
               //***IN FACT, IT HAS BEEN DEFINED IN THE OUTER {}
               //param_reference = log(dioCtrlInfo->getParam());
               //double residualEnergy = dioCtrlInfo->getResidualEnergy();
               //recordScalar("residuEnergy", residualEnergy);
               //double rssi = (5.0-param_reference)/20;

               //***BELOW IS FOR OF FUZZY, UNCOMMENT WHEN NECESSARY
               //double metrics_plus = dioCtrlInfo->getResidualEnergy();


               //***************************************
               //metrics = of0.counterCal(srID, metrics_t, NumDio, rssi);  //objective fun3
               //metrics = of2.counterCal(srID, metrics_t, NumDio); //objective fun2
               //double metrics_cur_etx = off.addictiveMetrics_t(srID, metrics_t_etx, NumDio);  //objective fun fuzzy
               //recordScalar("cur_etx", metrics_cur_etx);
               //metrics = off.counterCal(srID, metrics_cur_etx, metrics_plus); //objective fun fuzzy
               //metrics = of1.counterCal(srID, NumDio);  //objective fun1
               //EV<<"count metrics:"<<metrics<<endl;
               //recordScalar("metrics_t", metrics);
               IsJoined = true;
               NofJoined += 1;
               EV<<"The DODAG topology already has "<<NofJoined<<" nodes."<<endl;
               char buf2[100];
               sprintf(buf2, "JOINED!");
               host -> getDisplayString().setTagArg("t", 0, buf2);
               EV<<buf1<<endl;
               host -> bubble(buf1);
               of0.UpdateParent(metrics, dio->getSrID(), dio->getInitialSrcAddr(),dio->getRank(), dio->getInitialSrcMACAddr());
               EV<<"My first time joining a DODAG\nRank of me(node:"<<NodeID<<") is:"<<of0.Rank<<endl;
               ...
               if(NofJoined == numNodes)
               {
                    char buf3[100];
                    sprintf(buf3, "DODAG FINISHED with last node!");
                    char buf4[50];
                    sprintf(buf4, "last node joined!");
                    host->getDisplayString().setTagArg("t", 0, buf4);
                    EV << buf3 << endl;
                    recordScalar("DODAG Construction Finished Time",simTime());
                    //finish();
               }
               processDIOForRouterUpdates(dio,dioCtrlInfo);
               recordScalar("First join time",simTime());

           }
           else//IsJoined //The node has already joined the DODAG, any update is what I am caring aobut
           {
//               if(abs(param_reference - log(dioCtrlInfo->getParam())) >= 2.0)
//               {
//                   param_reference = param_reference*0.8 + log(dioCtrlInfo->getParam())*0.2;
//               }
//               else
//               {
//                   param_reference = param_reference*0.5 + log(dioCtrlInfo->getParam())*0.5;
//               }
//
//               double rssi = (5.0-param_reference)/20;
               //double residualEnergy = dioCtrlInfo->getResidualEnergy();
               //recordScalar("residuEnergy", residualEnergy);
               EV<<"I was already a member of DODAG."<<endl;
               EV<<"Rank according to previous of0:"<<of0.Rank<<"Rank updated by DIO:"<<dio->getRank()+1<<endl;

               if(dio->getRank() <= of0.Rank - 1)
               //dio->getRank() <= of0.Rank - 1(objective function 0)
               //dio->getRank() <= of0.Rank (objective function 1, objective function 2&3)
               //dio->getRank() <= off.Rank (objective function fuzzy)
               {
                   DIO_c = 0;
                   //!!!!*****************
                   //metrics = of0.counterCal(srID, metrics_t, NumDio, rssi);//objectivefun3
                   //metrics = of2.counterCal(srID, metrics_t, NumDio); //objective fun2
                   //metrics = of1.counterCal(srID,NumDio); //objectivefun1
                   ...
                   //recordScalar("metrics_t", metrics);
                   switch(of0.IsParent(dio->getInitialSrcAddr(),dio->getRank()))
                   {
                      case ObjectiveFun0::NOT_EXIST:
                          EV<<"not exist"<<endl;
                          of0.UpdateParent(metrics, dio->getSrID(), dio->getInitialSrcAddr(),dio->getRank(), dio->getInitialSrcMACAddr());
                          EV<<"Rank of me(node:"<<NodeID<<") is:"<<of0.Rank<<endl;
                          break;
                      case ObjectiveFun0::SHOULD_BE_UPDATED:
                          EV<<"should be updated"<<endl;
                          of0.UpdateParent(metrics, dio->getSrID(), dio->getInitialSrcAddr(),dio->getRank(), dio->getInitialSrcMACAddr());
                          EV<<"Rank of me(node:"<<NodeID<<") is:"<<of0.Rank<<endl;
                          EV<<"Metric of me(node:"<<NodeID<<") is:"<<of0.metrics<<endl;
                          break;
                      case ObjectiveFun0::EXIST:
                          EV<<"exist"<<endl;
                          EV<<"Rank of me(node:"<<NodeID<<") is:"<<of0.Rank<<endl;
                          of0.UpdateParent(metrics, dio->getSrID(), dio->getInitialSrcAddr(),dio->getRank(), dio->getInitialSrcMACAddr());
                          break;
           ...
           else //rank > current parent rank
           {
                EV<<"Ignore the dio message!!\nRank of me is:"<<of0.Rank<<endl;
                DIO_Supressed++;
                DIO_c++;
                if(DIO_c >= DIO_Redun)
                    DIO_c = DIO_Redun;
       ...
       if(simTime() > 50)
           recordScalar("DIO Supressed", DIO_Supressed);

       rankchangeLog.record(of0.Rank);
       parentLog.record(of0.PRNodeID);
       dio_c.record(DIO_c);
       //updateNodeState(src,of0.Rank);
   }
   destMACAddress = of0.prfMACparent;

   EV<<"My preferred parent MAC address is : "<<destMACAddress<<endl;

//       for (int i = 0; i < ift->getNumInterfaces(); i++)
//       {
//           InterfaceEntry *iedao = ift->getInterface(i);
//           createDAOTimer(iedao);
//       }
   delete dioCtrlInfo;
   delete dio;
}

void IPv6NeighbourDiscovery::processDIOForRouterUpdates(DIO *dio,
        IPv6ControlInfo *dioCtrlInfo)
{
    EV << "Processing DIO for Router Updates\n";
        ...
        IPv6Address dioSrcAddr = dioCtrlInfo->getSrcAddr();
        InterfaceEntry *ie = ift->getInterfaceById(dioCtrlInfo->getInterfaceId());
        ...
        Neighbour *neighbour = neighbourCache.lookup(dioSrcAddr, ifID);



        if (neighbour == NULL)
        {
            EV << "Neighbour Cache Entry does not contain DIO's source address\n";

                neighbour = neighbourCache.RPLaddRouter(dioSrcAddr, ifID,
                    dio->getInitialSrcMACAddr());

                //According to Greg, we should add a default route for hosts as well!
                //********************************
                //********************************
                //if change of - need modificaitons.
                rt6->RPLaddDefaultRoute(dioSrcAddr, ifID, metrics, dio->getRank());
        }
        else
        {
            ...
            if (dio->getInitialSrcMACAddr().isUnspecified() == false &&
                    neighbour->macAddress.equals(dio->getInitialSrcMACAddr()) == false)
                neighbour->macAddress = dio->getInitialSrcMACAddr();
            rt6->RPLaddDefaultRoute(dioSrcAddr, ifID, metrics, dio->getRank());
                    //of0.counterCal(dio->getSrID())/dio->getNumSent());
        }

}

void IPv6NeighbourDiscovery::createDIOTimer(InterfaceEntry *ie)
{

        DIOTimer = new cMessage("sendDIO", MK_SEND_DIO);
        DIOTimer->setContextPointer(ie);
        DIOIfEntry *dioIfEntry = new DIOIfEntry();
        dioIfEntry->interfaceId = ie->getInterfaceId();
        dioIfEntry->numDIOSent = NumSelfDIO;
        dioIfEntry->dioTimeoutMsg = DIOTimer;

        DIONextInt = DIOIntMin;
        DIO_c = 0;
        simtime_t nextScheduledTime = scheduleNextDIOTransmissionInconsistency(DIOTimer);

        dioIfEntry->nextScheduledDIOTime = nextScheduledTime;
        dioIfList.insert(dioIfEntry);

}



simtime_t IPv6NeighbourDiscovery::scheduleNextDAOTransmission(cMessage *DAOTimer)
{
    simtime_t delayDAOTimer = 0.1;
    simtime_t nextScheduledTime = simTime() + delayDAOTimer;
    EV << "delayDAOTimer: " << delayDAOTimer << endl;
    EV << "DAO next scheduled time: " << nextScheduledTime << endl;
    //now we schedule the msg for whatever time that was derived

    scheduleAt(nextScheduledTime, DAOTimer);
    return nextScheduledTime;
}


DAO *IPv6NeighbourDiscovery::createAndSendDAOPacket(const IPv6Address& destAddr, InterfaceEntry *ie)
{

    EV << "Create and send DAO invoked!\n";
    //Must use link-local addr. See: RFC2461 Section 6.1.2
    //IPv6Address sourceAddr = ie->ipv6Data()->getLinkLocalAddress();
    IPv6Address myIPv6Address = ie->ipv6Data()->getPreferredAddress(); //Interface Address
    DAO *dao = new DAO("DAOpacket");
    dao->setInitialSrcAddr(myIPv6Address);//PR_f,0,0,myNetwAddr));
    dao->setFinalDestAddr(destAddr);
    EV<<"\n"<<dao->getInitialSrcAddr()<<endl;
    dao->setVersionNumber(1) ;
    dao->setFinalDestMACAddr(destMACAddress);
    dao->setDODAGID(sinkIPv6Address) ;

    sendDAOPacketToIPv6Module(dao, destAddr, destMACAddress, myIPv6Address, ie->getInterfaceId());
    return dao;
}
void IPv6NeighbourDiscovery::sendDAO(cMessage *DAOTimer1)
{
    InterfaceEntry *ie = (InterfaceEntry *)DAOTimer1->getContextPointer();
    EV<<"Message Kind for check:"<<DAOTimer1->getKind()<<endl;
    DAOIfEntry *daoIfEntry = fetchDAOIfEntry(ie);
    IPv6Address destAddr = of0.prfparent;
    createAndSendDAOPacket(destAddr, ie);
    daoIfEntry->numDAOSent++;

    /*For the first few advertisements (up to MAX_INITIAL_RTR_ADVERTISEMENTS)
    sent from an interface when it becomes an advertising interface,*/
    EV << "Num DAO sent is: " << daoIfEntry->numDAOSent << endl;

    simtime_t nextScheduledTime = scheduleNextDAOTransmission(DAOTimer1);
    daoIfEntry->nextScheduledDAOTime = nextScheduledTime;
}
void IPv6NeighbourDiscovery::processDAOPacket(DAO *dao,IPv6ControlInfo *daoCtrlInfo)
{
    cModule* host = findHost();
    char buf0[50];
    sprintf(buf0,"Receiving DAO.\n");
    host->bubble(buf0);
   // InterfaceEntry *ie = ift->getInterfaceById(daoCtrlInfo->getInterfaceId());

    //processDAOForRouterUpdates(dao, daoCtrlInfo); //See RFC2461: Section 6.3.4

    //delete daoCtrlInfo;
    //delete dao;
}

void IPv6NeighbourDiscovery::processDAOForRouterUpdates(DAO *dao,
        IPv6ControlInfo *daoCtrlInfo)
{
    EV << "Processing DAO for Router Updates\n";
    //RFC2461: Section 6.3.4
    //Paragraphs 1 and 2 omitted.


    IPv6Address daoSrcAddr = daoCtrlInfo->getSrcAddr();
    InterfaceEntry *ie = ift->getInterfaceById(daoCtrlInfo->getInterfaceId());
    int ifID = ie->getInterfaceId();
    ...
    the list, and initialize its invalidation timer value from the advertisement's
    Router Lifetime field.*/
    Neighbour *neighbour = neighbourCache.lookup(daoSrcAddr, ifID);
    ...
    if (neighbour == NULL)
    {
        ...
        neighbour = neighbourCache.addRouter(daoSrcAddr, ifID,
                dao->getInitialMACAddr(),
                simTime() + dao->getRouterLifetime());

        //According to Greg, we should add a default route for hosts as well!
        rt6->addDefaultRoute(daoSrcAddr, ifID, simTime() + dao->getRouterLifetime());
    ...
    else
    {
        ...
        //If a cache entry already exists and is updated with a different link-
        //layer address the reachability state MUST also be set to STALE.
        if (dao->getInitialMACAddr().isUnspecified() == false &&
                neighbour->macAddress.equals(dao->getInitialMACAddr()) == false)
            neighbour->macAddress = dao->getInitialMACAddr();
        ...
        neighbour->routerExpiryTime = simTime()+dao->getRouterLifetime();

        /*- If the address is already present in the host's Default Router List
        and the received Router Lifetime value is zero, immediately time-out the
        entry as specified in Section 6.3.5.*/
        if (dao->getRouterLifetime() == 0)
        {
            EV << "DAO's router lifetime is ZERO. Timing-out entry.\n";
            timeoutDefaultRouter(daoSrcAddr, ifID);
        }
    }
...
void IPv6NeighbourDiscovery::createDAOTimer(InterfaceEntry *ie)
{

        cMessage *DAOTimer = new cMessage("sendDAO", MK_SEND_DAO);
        DAOTimer->setContextPointer(ie);
        DAOIfEntry *daoIfEntry = new DAOIfEntry();
        daoIfEntry->interfaceId = ie->getInterfaceId();
        daoIfEntry->numDAOSent = 0;
        daoIfEntry->daoTimeoutMsg = DAOTimer;
        simtime_t nextScheduledTime = scheduleNextDAOTransmission(DAOTimer);

        daoIfEntry->nextScheduledDAOTime = nextScheduledTime;
        daoIfList.insert(daoIfEntry);

}

simtime_t IPv6NeighbourDiscovery::scheduleNextDISTransmission(cMessage *DISTimer)
{
    simtime_t interval = uniform(0.5,2.0);
    simtime_t nextScheduledTime = simTime() + interval;
    EV << "Interval: " << interval << endl;
    EV << "Next scheduled time: " << nextScheduledTime << endl;
    //now we schedule the msg for whatever time that was derived

    scheduleAt(nextScheduledTime, DISTimer);
    return nextScheduledTime;
}

DIS *IPv6NeighbourDiscovery::createAndSendDISPacket(const IPv6Address& destAddr, InterfaceEntry *ie)
{

    EV << "Create and send DIS invoked!\n";
    IPv6Address myIPv6Address = ie->ipv6Data()->getPreferredAddress(); //Interface Address
    DIS *dis = new DIS("DISpacket");
    dis->setInitialSrcAddr(myIPv6Address);//PR_f,0,0,myNetwAddr));
    EV<<"\n"<<dis->getInitialSrcAddr()<<endl;
    dis->setFinalDestAddr(destAddr) ;
    dis->setVersionNumber(1) ;
    dis->setDODAGID(sinkIPv6Address) ;
    EV<<"DODAGID:"<<dis->getDODAGID()<<endl;

    sendPacketToIPv6Module(dis, destAddr, myIPv6Address, ie->getInterfaceId());
    return dis;
}


void IPv6NeighbourDiscovery::sendDIS(cMessage *DISTimer1)
{
    InterfaceEntry *ie = (InterfaceEntry *)DISTimer1->getContextPointer();
    EV<<"Message Kind for check:"<<DISTimer1->getKind()<<endl;
    DISIfEntry *disIfEntry = fetchDISIfEntry(ie);
    cModule* host = findHost();
    char buf0[50];
    if(!IsJoined)
    {
        EV<<"Sending DIS!"<<endl;
        IPv6Address destAddr = IPv6Address::ALL_ROUTERS_2; //IPv6Address("FF02::1A");
        createAndSendDISPacket(destAddr, ie);
        disIfEntry->numDISSent++;

        /*For the first few advertisements (up to MAX_INITIAL_RTR_ADVERTISEMENTS)
         sent from an interface when it becomes an advertising interface,*/
        EV << "Num DIS sent is: " << disIfEntry->numDISSent << endl;

        simtime_t nextScheduledTime = scheduleNextDISTransmission(DISTimer1);
        disIfEntry->nextScheduledDISTime = nextScheduledTime;
    }
    else
    {
        sprintf(buf0,"I am already a member of DODAG.\nDelete DIS.");
        host->bubble(buf0);
        EV<<"I am already a member of DODAG.\nDelete DIS."<<endl;
        cancelAndDelete(DISTimer1);
    }
...
void IPv6NeighbourDiscovery::processDISPacket(DIS *dis,IPv6ControlInfo *disCtrlInfo)
{
    cModule* host = findHost();
    char buf0[50];
    if(IsJoined){
        sprintf(buf0,"Receiving DIS.\nTrickle timer reset.");
        host->bubble(buf0);
        EV<<"Receving a DIS."<<endl;
        TrickleReset();
    }
    else{
        sprintf(buf0,"Not a member of DODAG.\nIgnore DIS.");
        host->bubble(buf0);
        EV<<"I am not a member of DODAG.\nIgnore dis."<<endl;
        cancelAndDelete(dis);
    }
}
void IPv6NeighbourDiscovery::createDISTimer(InterfaceEntry *ie)
{
    cMessage *DISTimer = new cMessage("sendDIS", MK_SEND_DIS);
    DISTimer->setContextPointer(ie);
    DISIfEntry *disIfEntry = new DISIfEntry();
    disIfEntry->interfaceId = ie->getInterfaceId();
    disIfEntry->numDISSent = 0;
    disIfEntry->disTimeoutMsg = DISTimer;
    simtime_t nextScheduledTime = scheduleNextDISTransmission(DISTimer);
    disIfEntry->nextScheduledDISTime = nextScheduledTime;
    disIfList.insert(disIfEntry);

}
cModule* IPv6NeighbourDiscovery::findHost(void)
{
...
void IPv6NeighbourDiscovery::processNDMessage(ICMPv6Message *msg, IPv6ControlInfo *ctrlInfo)
{
    ...
    else if (dynamic_cast<DIO *>(msg))
       {
           DIO *dio = (DIO *)msg;
           EV<<"DIO received!"<< endl;
           processDIOPacket(dio, ctrlInfo);
    }
    else if (dynamic_cast<DAO *>(msg)) {
        DAO *dao = (DAO *) msg;
                EV << "DAO received!" << endl;
                processDAOPacket(dao, ctrlInfo);
    }
    else if (dynamic_cast<DIS *>(msg))
       {
            DIS *dis = (DIS *) msg;
            EV << "DIS received!" << endl;
            processDISPacket(dis, ctrlInfo);
       }
...
void IPv6NeighbourDiscovery::processIPv6Datagram(IPv6Datagram *msg)
{
    ...
    //IPv6Address nextHopAddr = RPLdetermineNextHop(msg->getDestAddress(), nextHopIfID);
    IPv6Address nextHopAddr = RPLdetermineNextHop(msg->getDestAddress(), nextHopIfID);
    if (nextHopIfID == -1)
    {
...
IPv6NeighbourDiscovery::DIOIfEntry *IPv6NeighbourDiscovery::fetchDIOIfEntry(InterfaceEntry *ie)
{
   for (DIOIfList::iterator it = dioIfList.begin(); it != dioIfList.end(); it++)
   {
       DIOIfEntry *dioIfEntry = (*it);
       if (dioIfEntry->interfaceId == ie->getInterfaceId())
       {
           return dioIfEntry;
       }
   }
...
IPv6NeighbourDiscovery::DAOIfEntry *IPv6NeighbourDiscovery::fetchDAOIfEntry(InterfaceEntry *ie)
{
   for (DAOIfList::iterator it = daoIfList.begin(); it != daoIfList.end(); it++)
   {
       DAOIfEntry *daoIfEntry = (*it);
       if (daoIfEntry->interfaceId == ie->getInterfaceId())
       {
           return daoIfEntry;
       }
   }
...
IPv6NeighbourDiscovery::DISIfEntry *IPv6NeighbourDiscovery::fetchDISIfEntry(InterfaceEntry *ie)
{
   for (DISIfList::iterator it = disIfList.begin(); it != disIfList.end(); it++)
   {
       DISIfEntry *disIfEntry = (*it);
       if (disIfEntry->interfaceId == ie->getInterfaceId())
       {
           return disIfEntry;
       }
   }
...
const MACAddress& IPv6NeighbourDiscovery::RPLresolveNeighbour(const IPv6Address& nextHop, int interfaceId)
{
    Enter_Method("resolveNeighbor(%s,if=%d)", nextHop.str().c_str(), interfaceId);

    Neighbour *nce = neighbourCache.lookup(nextHop, interfaceId);
    if (!nce)
            return MACAddress::UNSPECIFIED_ADDRESS;    //else the entry is REACHABLE and no further action is required here.
    return nce->macAddress;
}
...
IPv6Address IPv6NeighbourDiscovery::RPLdetermineNextHop(
    const IPv6Address& destAddr , int& outIfID)
{
    IPv6Address nextHopAddr;
    IPv6Route *route = rt6->getRouteList();
...
void IPv6NeighbourDiscovery::dropQueuedPacketsAwaitingAR(Neighbour *nce)
{
    ...
    EV << "Removing neighbor cache entry" << endl;
    neighbourCache.remove(nceKey->address, nceKey->interfaceID);
}
...
{
    EV<<"packet sent to ipv6 module"<<endl;
...
void IPv6NeighbourDiscovery::sendDAOPacketToIPv6Module(cMessage *msg, const IPv6Address& destAddr,
        const MACAddress& destMACAddr, const IPv6Address& srcAddr, int interfaceId)
{
    EV<<"packet sent to ipv6 module"<<endl;
    IPv6ControlInfo *controlInfo = new IPv6ControlInfo();
    controlInfo->setProtocol(IP_PROT_IPv6_ICMP);
    controlInfo->setDestAddr(destAddr);
    controlInfo->setDestMACAddr(destMACAddr);
    EV<<"My dest mac address is : "<<destMACAddress<<endl;
    controlInfo->setSrcAddr(srcAddr);
    controlInfo->setHopLimit(255);
    controlInfo->setInterfaceId(interfaceId);
    msg->setControlInfo(controlInfo);
...
//This function is for of0, which related to hop count
...
//
//                //According to Greg, we should add a default route for hosts as well!
//                rt6->RPLaddDefaultRoute(dioSrcAddr, ifID, dio->getRank());
//        }
//        else
//        {
//            neighbour->isRouter = true;
...
//if here we use of, we need to take counter into account
//end of this new function
#ifndef WITH_xMIPv6
        ...
        IPv6NDPrefixInformation& prefixInfo, InterfaceEntry *ie)
{
    ...
    /*d) If the prefix advertised does not match the prefix of an address already
         ...
         it to the list) by combining the advertised prefix with the link锟絪
...
void IPv6NeighbourDiscovery::updateNodeState(const IPv6Address& destAddr,int Rank)
{
    if(nodeCache.find(destAddr) == nodeCache.end())
    {
        NodeState& ns = nodeCache[destAddr];
        ns.Rank = Rank;
        ns.prfParent = of0.prfparent;
        ns.JoiningTime = simTime();
    }
    else
    {
        NodeState& ns = nodeCache.find(destAddr)->second;
        ns.Rank = Rank;
        ns.prfParent = of0.prfparent;
        ns.JoiningTime = simTime();
    }
    recordScalar("preferred parent",of0.PRNodeID);
    recordScalar("rank",of0.Rank);
}
