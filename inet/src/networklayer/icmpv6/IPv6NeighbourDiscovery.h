...
#include "ObjectiveFun0.h"//******"ObjectiveFun2/3.h"
//#include "/Users/apple/Desktop/omnetpp-4.6/samples/MiXiM/src/base/utils/MacToNetwControlInfo.h"
#include "/Users/apple/Documents/omnetpp-4.6/samples/MiXiM/src/modules/power/battery/SimpleBattery.h"
#include "/Users/apple/Documents/omnetpp-4.6/samples/MiXiM/src/base/utils/HostState.h"
...
public:
     ...
     */
    const MACAddress& resolveNeighbour(const IPv6Address& nextHop,
            int interfaceId);

    const MACAddress& RPLresolveNeighbour(const IPv6Address& nextHop,
                int interfaceId);
    /**
     ...
     * running.
     */
    virtual void reachabilityConfirmed(const IPv6Address& neighbour,
            int interfaceId);

protected:

    cOutVector dio_c;
    cOutVector rankchangeLog;
    cOutVector DIONumchangeLog;
    cOutVector parentLog;
    ...
    RoutingTable6 *rt6;
    ICMPv6 *icmpv6;
    ObjectiveFun0 of0;   //******ObjectiveFun2 of2;  //******ObjectiveFun3 of3;
    ...
    int DIORedun;
    int DIOIntDoubl;
    double DIOIMaxLength;
    int NodeID;
    bool IsJoined;
    bool DISenabled;
    simtime_t MsgEnding;
    double param_reference;   //defined for of3 which use this parameter to serve as a modifier of the metrics
    ...
    typedef std::map<IPv6Address, NodeState> NodeCache;
    NodeCache nodeCache;
    ...
    struct DIOIfEntry {
        int interfaceId;
        unsigned int numDIOSent; //number of DIO sent since start of sim
        simtime_t nextScheduledDIOTime; //stores time when next RA will be sent.
        cMessage *dioTimeoutMsg; //the message to cancel when resetting RA timer
    };
    typedef std::set<DIOIfEntry*> DIOIfList;

    struct DAOIfEntry {
                int interfaceId;
                unsigned int numDAOSent; //number of Router Advertisements sent since start of sim
                simtime_t nextScheduledDAOTime; //stores time when next RA will be sent.
                cMessage *daoTimeoutMsg; //the message to cancel when resetting RA timer
            };
        typedef std::set<DAOIfEntry*> DAOIfList;

    struct DISIfEntry {
        int interfaceId;
        unsigned int numDISSent; //number of Router Advertisements sent since start of sim
        simtime_t nextScheduledDISTime; //stores time when next RA will be sent.
        cMessage *disTimeoutMsg; //the message to cancel when resetting RA timer
    };
    typedef std::set<DISIfEntry*> DISIfList;
    ...
    int DIO_Redun;
    int DIO_Supressed;
    int NumSelfDIO; //record the node itself sending DIO number
...
protected:
    ...
    virtual IPv6NeighbourDiscovery::AdvIfEntry *fetchAdvIfEntry(
            InterfaceEntry *ie);
     ...
     *  interface.
     */
    virtual IPv6Address determineNextHop(const IPv6Address& destAddr,
            int& outIfID);
    virtual IPv6Address RPLdetermineNextHop(const IPv6Address& destAddr,
            int& outIfID);
     ...
     */
    virtual void timeoutPrefixEntry(const IPv6Address& destPrefix,
            int prefixLength);
    /**
     *  RFC 2461: Section 6.3.5
     ...
     */
    virtual void sendPacketToIPv6Module(cMessage *msg,
            const IPv6Address& destAddr, const IPv6Address& srcAddr,
            int interfaceId);
    /**
         *  Create control info and assigns it to a msg. Returns a copy of the
         *  msg with the control info. for DAO.
         */
    virtual void sendDAOPacketToIPv6Module(cMessage *msg,
                const IPv6Address& destAddr, const MACAddress& destMACAddr, const IPv6Address& srcAddr,
                int interfaceId);
     ...
     */
    virtual void initiateDAD(const IPv6Address& tentativeAddr,
            InterfaceEntry *ie);
     ...
     */
    virtual void makeTentativeAddressPermanent(const IPv6Address& tentativeAddr,
            InterfaceEntry *ie);
    ...
    /************DIO related stuff*******added by seer*********************/
    virtual void updateNodeState(const IPv6Address& destAddr, int Rank);
    virtual simtime_t scheduleNextDIOTransmissionInconsistency(cMessage *DIOTimer);
    virtual simtime_t scheduleNextDIOTransmissionConsistency(cMessage *DIOTimer);

    virtual cModule* findHost(void);
    virtual void createDIOTimer(InterfaceEntry *ie);
    virtual void sendDIO(cMessage *msg);
    virtual void processDIOPacket(DIO *dio, IPv6ControlInfo *dioCtrlInfo);
    virtual IPv6NeighbourDiscovery::DIOIfEntry *fetchDIOIfEntry(
            InterfaceEntry *ie);
    virtual DIO *createAndSendDIOPacket(const IPv6Address& destAddr,
            InterfaceEntry *ie);
    virtual void TrickleReset();
    virtual void processDIOForRouterUpdates(DIO *dio,
                IPv6ControlInfo *dioCtrlInfo);

    /************DAO related stuff*******added by seer*********************/
    virtual void createDAOTimer(InterfaceEntry *ie);
    virtual simtime_t scheduleNextDAOTransmission(cMessage *DAOTimer);
    virtual void sendDAO(cMessage *msg);
    virtual DAO *createAndSendDAOPacket(const IPv6Address& destAddr,
            InterfaceEntry *ie);
    virtual IPv6NeighbourDiscovery::DAOIfEntry *fetchDAOIfEntry(
            InterfaceEntry *ie);
    virtual void processDAOPacket(DAO *dao, IPv6ControlInfo *daoCtrlInfo);
    virtual void processDAOForRouterUpdates(DAO *dao,
            IPv6ControlInfo *daoCtrlInfo);

    /************DIS related stuff*******added by seer*********************/
    virtual void createDISTimer(InterfaceEntry *ie);
    virtual simtime_t scheduleNextDISTransmission(cMessage *DIOTimer);
    virtual void sendDIS(cMessage *msg);
    virtual DIS *createAndSendDISPacket(const IPv6Address& destAddr,
                InterfaceEntry *ie);
    virtual IPv6NeighbourDiscovery::DISIfEntry *fetchDISIfEntry(
                InterfaceEntry *ie);
    virtual void processDISPacket(DIS *dis, IPv6ControlInfo *dioCtrlInfo);

    /************Router Advertisment Stuff*********************************/
    virtual IPv6RouterAdvertisement *createAndSendRAPacket(
    ...
    virtual void processRAPrefixInfoForAddrAutoConf(IPv6NDPrefixInformation& prefixInfo,
            InterfaceEntry *ie);
#else /* WITH_xMIPv6 */
    virtual void processRAPrefixInfoForAddrAutoConf(
            IPv6NDPrefixInformation& prefixInfo, InterfaceEntry *ie,
            bool hFlag = false); // overloaded method - 3.9.07 CB
#endif /* WITH_xMIPv6 */
...
#ifdef WITH_xMIPv6
    IPv6NeighbourAdvertisement *createAndSendNAPacket(
