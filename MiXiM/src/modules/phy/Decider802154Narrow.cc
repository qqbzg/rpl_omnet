	...
	while(curTime < end) {
		...
		if (noErrors) {
            ...
            if(recordStats) {
              ...
              rssilog.record(calcChannelSenseRSSI(start, end).first);
              recordSnr += MW2DBM(snr);
              EV<<"recordSnr:"<<recordSnr<<" num:"<<snrlog.getValuesStored()<<"avgRecordSNR:"<<recordSnr/snrlog.getValuesStored()<<endl;
            }
