...
void csma::initialize(int stage) {
	...
	debugEV<<"\nUseMacAck = "<<useMACAcks<<"\t  rxSetupTime = "<<rxSetupTime<<"\t  ccaDetectionTime = "<<ccaDetectionTime<<"\t  queueLength = "<<queueLength<<"\tMinBE = "<<macMinBE<<"\tMaxBE = "<<macMaxBE<<endl;
}
...
void csma::handleUpperMsg(cMessage *msg) {
	...
	debugEV <<"pkt encapsulated, length: " << macPkt->getByteLength() << "\n";
