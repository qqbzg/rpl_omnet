...
cPacket *CSMA802154::decapsMsg(macpkt_ptr_t macPkt) {
    ...
    double residualEnergy = batt->GetResidualCapacity();
    double nominalEnergy = batt->GetNominalCapacity();
	// get bit error rate
	...
	double ber = result->getBER();  //why 0?
	double rssi = result->getRSSI();
    EV<<"result->rssi:"<<result->getRSSI()<<endl;
    EV<<"result->snr:"<<result->getSnr()<<endl;
    EV<<"result->ber:"<<result->getBER()<<endl;
	...
	EV<<msg->getControlInfo()->getClassName()<<endl;
	cInfo->setBitErrorRate(ber);
	cInfo->setRSSI(rssi);
	cInfo->setresidualEnergy(residualEnergy/nominalEnergy);
	recordScalar("cInfo->getresidualEnergy",cInfo->getresidualEnergy());
    recordScalar("cInfo->getrssi",cInfo->getRSSI());
    EV<<"cInfo RSSI:"<<cInfo->getRSSI()<<endl;
	EV<<"msg class:"<<msg->getClassName()<<endl;
	return msg;
}
