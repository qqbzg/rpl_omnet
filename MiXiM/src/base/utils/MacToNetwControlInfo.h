...
protected:
    ...
    /** Added 9th Jan 2016 to get access to battery in MacToNetwContr*/
    double rssi;
...
public:
    ...
    MacToNetwControlInfo(const LAddress::L2Type& lastHop, double ber = 0, double residu = 0, double rssi = 0):
        bitErrorRate(ber),
        lastHopMac(lastHop),
        residu(residu),
        rssi(rssi)
    ...
    /**
     * @brief Returns the signal noise rate.
     *
     ...
     * @param _residu The received signal strength
     */
    //void setSNR(double _snr) {
    //  snr = _snr;
    //}
    /**
    ...
    virtual void setresidualEnergy(double _residu) {
        residu = _residu;
    }
    /**Added 19th Jan
     *
